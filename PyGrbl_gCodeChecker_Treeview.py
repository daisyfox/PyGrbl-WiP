...
#        self.gCodeCheckerTable = ScrollTable(self, self.app)
        self.gCodeCheckerTable = Scroll_Treeview(self, self.app)
...
#                self.gCodeCheckerTable.fill_STable(self.app.gCodeData)
                self.gCodeCheckerTable.fill_treeview(self.app.gCodeData)
...
class Scroll_Treeview(Frame):
    def __init__(self, master, mainClass):
        ...
        self.tree = ttk.Treeview(self, columns = ("col1", "col2", "col3", "col4"))

        self.tree.column("#0", width = 20)        
        self.tree.column("col1", width = 200)
        self.tree.column("col2", width = 100)
        self.tree.column("col3", width = 100)
        self.tree.column("col4", width = 200)

        self.tree.heading("col1", text = "gCode IN")
        self.tree.heading("col2", text = "Result")
        self.tree.heading("col3", text = "Comment")
        self.tree.heading("col4", text = "gCode OUT")

        self.yScBar = ttk.Scrollbar(orient = VERTICAL, command = self.tree.yview)
        self.tree['yscroll'] = self.yScBar.set

        self.tree.grid(in_ = self, row = 0, column = 0, sticky = (N, S, W, E))
        self.yScBar.grid(in_ = self, row = 0, column = 1, sticky = (N, S))
        

    def fill_treeview(self, gcodeData):
        rows = len(gcodeData[0])
        ...
        for r in range(rows):
            ...
            col3 = gcodeData[2][r]
            col4 = gcodeData[3][r]
            self.tree.insert("", 'end', text = str(r), values = (col1, col2, col3, col4))
